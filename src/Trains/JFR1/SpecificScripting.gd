...
func _process(_delta: float) -> void:
	...
	if not is_ready:
		is_ready = true
		ready()
	get_node("../Cabin/DisplayMiddle/Display").update_display(Math.speed_to_kmh(player.speed), player.technicalSoll, player.door_left.is_closing(), player.door_right.is_closing(), player.is_doors_closing(), player.enforced_braking, player.automaticDriving, player.currentSpeedLimit, player.engine, player.reverser)
