...
func check_station(delta: float) -> void:
	...
	if speed != 0:
		...
		if is_in_station and is_all_doors_closed():
	...
	if not is_in_station and whole_train_in_station and is_all_doors_closed():
	...
	if (not is_in_station and (current_station_table_entry.stop_type == StopType.BEGINNING \
			or (whole_train_in_station and is_doors_opened_for_platform(current_station_node.platform_side)))):
#		if current_station_table_entry.stop_type == StopType.BEGINNING:
...
### Doors: ###
func _call_wagon_doors(fnSideAction: String) -> void:
	for wagon in wagonsI:
		wagon.call(fnSideAction)


func _open_side_doors(fnSideAction: String) -> void:
	if doors and speed == 0:
		_call_wagon_doors(fnSideAction)		


func _close_side_doors(fnSideAction: String) -> void:
	if doors:
		_call_wagon_doors(fnSideAction)
...
# TODO: review all requests to is_doors_closing - 
# Most likely we need is_doors_transiting for UI: should be checking is_open and is_opening
func is_doors_closing(door_side: int = DoorSide.UNASSIGNED) -> bool:
	match door_side:
		DoorSide.LEFT:
			return door_left.is_closing()
		DoorSide.RIGHT:
			return door_right.is_closing()
	return door_right.is_closing() or door_left.is_closing()

func is_doors_opened(door_side: int = DoorSide.UNASSIGNED) -> bool:
	match door_side:
		DoorSide.LEFT:
			return door_left.is_opened()
		DoorSide.RIGHT:
			return door_right.is_opened()
	return door_right.is_opened() and door_left.is_opened()

func is_doors_opened_for_platform(platform_side: int = PlatformSide.BOTH) -> bool:
	match platform_side:
		PlatformSide.LEFT:
			return is_doors_opened(DoorSide.LEFT)
		PlatformSide.RIGHT:
			return is_doors_opened(DoorSide.RIGHT)
		PlatformSide.BOTH:
			return is_doors_opened()
	# Platform requires no doors to operate
	return true

func is_all_doors_closed() -> bool:
	return door_left.is_closed() and door_right.is_closed()
...
func toggle_door_side(doorState: DoorState, fnOpen: String, fnClose: String) -> void:
	if doorState.is_closed() or doorState.is_opened():
...
func check_doors() -> void:
	...
	if Input.is_action_just_pressed("doorLeft"):
		toggle_door_side(door_left, "open_left_doors", "close_left_doors")
...
func check_for_player_help(delta: float) -> void:
	...
	if blockedAcceleration and accRoll > 0 and brakeRoll == 0 and is_all_doors_closed() and check_for_player_helpTimer2 > 10 and not is_in_station:
...
func updateSwitchOnNextChange():
	...
	if route_information.size() > route_index+1:
		var nextRail: Spatial = route_information[route_index + 1].rail
...
func jump_to_rail(rail: Spatial, distance: float, fwd: bool = true) -> void:
	set_speed_to_zero()
	...
	for index in range(route_information.size()):
		var baked_rail: Spatial = route_information[index].rail
